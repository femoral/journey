...
| `tlsRejectUnauthorized` | `boolean`      | `true`           | No       | Set to `false` to disable TLS certificate verification when `journey run` / `journey serve` fetch over HTTPS — useful for self-signed certs or corporate CAs Node's bundled store doesn't trust. Equivalent to passing `--insecure` on every run; prints one stderr warning per process. Don't ship `false` to CI.                                                   |
| `connectTimeoutMs`      | `integer >= 0` | `10000` (undici) | No       | Budget in ms for the connect phase — DNS + TCP + TLS handshake — of every request. `0` disables it. This is **not** the request timeout (`--timeout` / step `timeoutMs`), which can only fire once a connection exists; a host that is slow to resolve or handshake fails with `UND_ERR_CONNECT_TIMEOUT` regardless of it. `--connect-timeout` overrides this value. |
...
- `connectTimeoutMs` must be a non-negative integer.
