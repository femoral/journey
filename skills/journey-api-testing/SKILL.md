...
| `timeoutMs` | `number`                                                         | Per-step abort timeout; defaults to 60000 (`--timeout` sets the run-wide default, `0` disables). Covers the request once connected — the connect phase has its own budget (`--connect-timeout`, default 10s). No retry knob — stage a retry as its own step, or move flaky load-style stuff to k6.                                                                                                                                                                                               |
...
journey run --all --connect-timeout 30000         # raise the DNS+TCP+TLS budget (default 10s)
```
...
| `Connect Timeout Error (attempted address: host:443, timeout: 10000ms) (UND_ERR_CONNECT_TIMEOUT)` | The **connect** phase (DNS + TCP + TLS handshake) blew undici's 10s default. `--timeout` does not cover it — it only wraps the fetch once a connection exists, so `--timeout 0` changes nothing here.                              | Raise the connect budget: `journey run --connect-timeout 30000` (or `connectTimeoutMs` in `journey.config.json`; `0` disables). If it reproduces instantly rather than at ~10s, the host is unreachable — check VPN/DNS instead.                    |
