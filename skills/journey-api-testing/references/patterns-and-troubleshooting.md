...
| `connectTimeoutMs`      | `integer >= 0` | `10000` (undici) | No       | Connect-phase budget (DNS + TCP + TLS handshake) in ms; `0` disables. Distinct from the request timeout — `--timeout` / step `timeoutMs` cannot fire before a connection exists. `--connect-timeout` overrides it.              |
...
| `Connect Timeout Error (... timeout: 10000ms) (UND_ERR_CONNECT_TIMEOUT)`                               | Connect phase (DNS + TCP + TLS) exceeded undici's 10s default, which `--timeout` does not govern | `journey run --connect-timeout 30000`, or `connectTimeoutMs` in `journey.config.json`; `0` disables it. Failing well before ~10s means the host is simply unreachable. |
