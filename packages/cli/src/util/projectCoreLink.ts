...
// `import "@journey/core"` therefore needs the package present somewhere
// in the project's ancestor chain. Rather than asking users to `pnpm
// install` inside every project (the CLI already ships @journey/core as
// a sibling dep), we plant a symlink at `<projectDir>/node_modules/@journey/core`
// pointing at the CLI's bundled copy.
function locateJourneyCoreDir(): string {
  let dir = dirname(fileURLToPath(import.meta.url));
  for (let depth = 0; depth < 50; depth++) {
    const pkgJson = join(dir, "node_modules", "@journey", "core", "package.json");
  ...
  throw new Error("ensureProjectCoreLink: could not locate the CLI-bundled @journey/core");
}

let cachedCoreDir: string | undefined;

export async function ensureProjectCoreLink(projectDir: string): Promise<void> {
  const link = join(projectDir, "node_modules", "@journey", "core");
