  ...
  return {
    toBe(expected) {
      if (!Object.is(value, expected))
        fail("expected " + JSON.stringify(value) + " to be " + JSON.stringify(expected));
    },
    toEqual(expected) {
      if (!deepEqual(value, expected))
        fail("expected " + JSON.stringify(value) + " to equal " + JSON.stringify(expected));
    },
    toBeDefined() {
      if (value === undefined) fail("expected value to be defined");
    },
    toContain(expected) {
...
function __runSub(node, depth) {
  ...
  try {
    group(label, () => {
      __runNodes(childNodes, handle.name, depth + 1);
    });
  } finally {
